...
@include('estate::layouts.elements.title', ['title' => 'Statistiche Categorie Prodotti'])
        ...
        @foreach($data as $cat => $values)
            <div class="col-6 col-md-3">
                <div class="small-box bg-info">
                    <div class="inner">
                        <h6 class="mb-0">{{Areaseb\Estate\Models\Category::find($cat)->nome}}</h6>
                        <p class="mb-0">Venduti: {{$values['totali']}}</p>
                        <p class="mb-0">Fatturato: {{$values['fatturato']}}</p>
                    </div>
                    <a href="{{url('stats/categorie/'.$cat.'?year='.date('Y'))}}" class="small-box-footer">Dettagli <i class="fas fa-arrow-circle-right"></i></a>
                </div>
            </div>
        @endforeach
    </div>
...
@section('scripts')
    ...
    $('select.selectYear').on('change', function(){
        ...
        if(val != 'tutti')
        {
            window.location.href = baseURL+'stats/categorie?year='+val;
        }
        else
        {
            window.location.href = baseURL+'stats/categorie';
        }
    });
