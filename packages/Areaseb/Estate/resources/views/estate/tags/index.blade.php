...
@include('estate::layouts.elements.title', ['title' => 'Tipologie'])
                ...
                <div class="card-header bg-secondary-light">
                    <div class="card-tools">
                        <div class="btn-group" role="group">
                            <a class="btn btn-primary creable" href="#" data-model="Tag" data-field="name_it"><i class="fas fa-plus"></i> Crea Tipologia</a>
                        </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered table-striped">
                            ...
                            <tbody>
                                @foreach($tags as $tag)
                                    <tr id="row-{{$tag->id}}" data-model="Tag" data-id="{{$tag->id}}">
                                        <td class="property-editable" data-field="name_it">{{$tag->name_it}}</td>
                                        <td class="property-editable" data-field="name_en">{{$tag->name_en}}</td>
                                        <td>
                                            {{$tag->properties()->count()}}
                                        </td>
                                        <td>

                                            {!! Form::open(['method' => 'delete', 'url' => route('tags.destroy', $tag->id), 'id' => "form-".$tag->id]) !!}
                                                <button type="submit" id="{{$tag->id}}" class="btn btn-danger btn-icon btn-sm delete"><i class="fa fa-trash"></i></button>
                                            {!! Form::close() !!}
