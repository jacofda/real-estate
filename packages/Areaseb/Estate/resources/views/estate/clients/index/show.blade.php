                ...
                <div class="card-header bg-secondary-light">
                    ...
                    <div class="row">
                        ...
                        <div class="col-6 text-right">
                            <a href="{{route('companies.create')}}" class="btn btn-primary"><i class="fa fa-plus"></i> Crea Cliente</a>
                        </div>
                    </div>
                            ...
                            <tbody>
                                ...
                                <tbody>
                                    <tr v-for="(result,index) in results" class="result">
                                        <td><a v-bind:href="'{{url('companies')}}/' + result.id"> @{{result.rag_soc}} </a> </td>
                                        <td>
                                        ...
                                        <td>@{{result.clients[0].nome}}</td>
                                        <td>@{{result.contacts[0].origin}}</td>
                                        <td>@{{result.email}}</td>
                                        <td>@{{result.phone | international }}</td>
                                        <td>
                                            <a v-bind:href="'{{url('companies')}}/' + result.id" class="btn btn-warning btn-icon btn-sm"><i class="fa fa-edit"></i></a>
...
@section('scripts')
    ...
    const app = new Vue({
    	...
    	methods:{
    		search:function() {
    			this.searching = true;
    			fetch(`http://localhost/crm/public/companies?q=${encodeURIComponent(this.term)}`)
    			.then(res => res.json())
    			.then(res => {
